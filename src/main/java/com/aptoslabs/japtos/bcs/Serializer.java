...
public class Serializer {
    ...
    /**
     * Writes bytes directly to the buffer without length prefix.
     *
     * <p>This method is useful for custom serialization where you need
     * to write raw bytes without the standard BCS length prefix. Use with
     * caution as it bypasses the standard BCS format.</p>
     *
     * @param bytes the bytes to write directly
     * @throws IOException if writing to the buffer fails
     */
    public void writeBytesDirect(byte[] bytes) throws IOException {
        buffer.write(bytes);
    }
}
