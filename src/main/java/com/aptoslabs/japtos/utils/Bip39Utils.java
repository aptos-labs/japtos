...
/**
 * Utility class for BIP39 mnemonic generation from entropy.
 * This implementation follows the BIP39 specification for converting entropy to mnemonic phrases.
 */
public class Bip39Utils {
    ...
    /**
     * Converts an arbitrary input string to a BIP39 mnemonic as follows:
     * - Convert the input to UTF-8 bytes, then to hex, and slice to first 32 hex chars (16 bytes)
     * - Apply BIP39 entropy→mnemonic algorithm using the English wordlist
     * <p>
     * This mirrors the JS logic:
     * const entropyHex = Buffer.from(input).toString('hex').slice(0, 32);
     * entropyToMnemonic(entropyHex)
     */
    public static String entropyToMnemonic(String entropyInput) {
        // Convert input string to UTF-8 bytes → hex, keep first 16 bytes (32 hex chars)
        String hex = bytesToHex(entropyInput.getBytes(StandardCharsets.UTF_8));
        String entropyHex = hex.length() > 32 ? hex.substring(0, 32) : hex;

        // Pad to 32 hex characters if shorter (to match TypeScript behavior)
        while (entropyHex.length() < 32) {
            entropyHex += "0";
        }

        byte[] entropy = hexToBytes(entropyHex);

        // Validate entropy length as per BIP-39
        if (entropy.length < 16 || entropy.length > 32 || (entropy.length % 4) != 0) {
            throw new IllegalArgumentException("Invalid entropy length: " + entropy.length);
        }
        ...
        for (int i = 0; i < numWords; i++) {
            ...
            if (i > 0) mnemonicBuilder.append(' ');
            mnemonicBuilder.append(WORDLIST[index]);
        }
        return mnemonicBuilder.toString();
    }

    private static String deriveChecksumBits(byte[] entropy) {
        int ent = entropy.length * 8;
        int cs = ent / 32;
        byte[] hash = CryptoUtils.sha256(entropy);
        String hashBits = bytesToBinary(hash);
    ...
    private static String bytesToBinary(byte[] bytes) {
        ...
        for (byte b : bytes) {
            ...
            if (bin.length() < 8) {
                ...
                for (int i = 0; i < pad; i++) binary.append('0');
            }
            binary.append(bin);
        }
    ...
    private static byte[] hexToBytes(String hex) {
        int len = hex.length();
        if ((len & 1) == 1) {
            // pad right if odd length, to keep behavior similar to JS slicing
            hex = hex + "0";
            len++;
        }
