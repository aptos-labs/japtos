...
public class MultiKeyTests {
    ...
    @Test
    @DisplayName("Test enhanced from() method with automatic signer index detection")
    @Order(8)
    public void testEnhancedFromMethodWithAutomaticIndexDetection() throws Exception {
        // Create two accounts
        Ed25519Account account1 = Ed25519Account.generate();
        Ed25519Account account2 = Ed25519Account.generate();

        // Create public keys array with account2 first, then account1
        List<Ed25519PublicKey> publicKeys = Arrays.asList(
                account2.getPublicKey(),  // Index 0
                account1.getPublicKey()   // Index 1
        );
        ...
        assertNotNull(multiAccount.getAccountAddress());
        assertEquals(Account.SigningScheme.MULTI_ED25519, multiAccount.getSigningScheme());

        // Verify signer indices are correct
        List<Integer> signerIndices = multiAccount.getSignerIndices();
        assertEquals(1, signerIndices.size());
        assertEquals(1, signerIndices.get(0)); // account1 should be at index 1

        // Test signing a transaction
        RawTransaction transaction = new RawTransaction(
                multiAccount.getAccountAddress(),
                0L,
        ...
        assertEquals(1, multiAuth.getThreshold());

        // Verify the authenticator has the correct signer indices
        List<Integer> authSignerIndices = multiAuth.getSignerIndices();
